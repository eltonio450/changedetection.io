...
/**
 * @file
 * Defines Javascript behaviors on the ticketing products header.
 */

(function ($, Drupal, drupalSettings) {
  'use strict';

  Drupal.behaviors.productHeaderActions = {
    attach: function (context) {
      // Initiate more info toggle when DOM is ready.
      $('.more-info-trigger, .more-info-hide-button', context).once('hubber-product-more-info-init').on('click', function () {
        $('.more-info-wrapper').toggleClass('open');
        $('body').toggleClass('overflow-hidden');
      });

      this.generateMobileDropdownActions();
    },
    /**
     * Generates a dropdown menu with all possibles actions for mobile devices.
     */
    generateMobileDropdownActions: function () {
      const $headerWrapper = $('.ticket-header-info-wrapper');
      $headerWrapper.find('.js-mobile-actions-toggle').remove();

      const $moreInfoToggle = $headerWrapper.find('.more-info-trigger');
      const $actionsLinks = $headerWrapper.find('.js-action-link');
      if (!$moreInfoToggle.length && !$actionsLinks.length) {
        return;
      }
      if (!$actionsLinks.length) {
        $moreInfoToggle.removeClass('d-none d-lg-inline-flex');
        return;
      }
      $moreInfoToggle.addClass('d-none d-lg-inline-flex');
      let $dropDownMenu = $('<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMobileButton"></div>');
      if ($moreInfoToggle.length) {
        let $linkAction = $('<a class="dropdown-item js-show-more-info" href="#">' + $moreInfoToggle.text() + '</a>');
        $linkAction.appendTo($dropDownMenu);
        $dropDownMenu.on('click', '.js-show-more-info', function() {
          $moreInfoToggle.trigger('click');
        });
      }
      $actionsLinks.each(function() {
        $(this).clone(true).addClass('dropdown-item').appendTo($dropDownMenu);
      });
      let $dropDownWrapper = $('<div class="dropdown mobile-actions-wrapper js-mobile-actions-toggle"><button class="dropdown-toggle" type="button" id="dropdownMobileButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Menu</button></div>');
      $dropDownMenu.appendTo($dropDownWrapper);
      $dropDownWrapper.appendTo($headerWrapper);
    }
  };
})(jQuery, Drupal, drupalSettings);
;
