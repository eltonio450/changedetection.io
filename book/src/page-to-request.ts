...
export const pageToBody = (page: string): NoSelectionBody => {
    ...
    const form_token = root.querySelector('input[name="form_token"]')._attrs.value;
    const form_id = root.querySelector('input[name="form_id"]')._attrs.value;
    const form_build_id = root.querySelector('input[name="form_build_id"]')._attrs.value;
    return ({
        form_token,
        form_id,
        form_build_id,
